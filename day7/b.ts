...
async function B() {
  ...
  const isFiveOfAKind = (counts: Record<string, number>): number => {
    const js = counts["J"] || 0;
    const found = Object.values(counts).filter((count) => count === 5 - js);
  ...
  const isFourOfAKind = (counts: Record<string, number>): number => {
    ...
    const found = Object.values(counts).filter((count) => count === 4 - js);

    if ((js == 2 && found.length == 2) || found.length == 1) {
      return 5;
    }
    return 0;
  };
  ...
  const isFullHouse = (counts: Record<string, number>): number => {
    ...
    const found3 = Object.values(counts).filter((count) => count === 3 - js);
  ...
  const isThreeOfAKind = (counts: Record<string, number>): number => {
    ...
    const found = Object.values(counts).filter((count) => count === 3 - js);
  ...
  const isTwoPair = (counts: Record<string, number>): number => {
    ...
    const found2a = Object.values(counts).filter((count) => count === 2 - js);
    const found2b = Object.values(counts).filter((count) => count === 2);

    if ((found2a.length == 1 && found2b.length == 1) || found2b.length == 2) {
      return 2;
    }
    return 0;
  };
  ...
  const isOnePair = (counts: Record<string, number>): number => {
    ...
    const found = Object.values(counts).filter((count) => count === 2 - js);
  ...
  function process(input: string): number {
    ...
    const winnings = hands
      ...
      .map((a) => {
        console.log(a, rank(a.rate, a.hand));
        return a;
      })
